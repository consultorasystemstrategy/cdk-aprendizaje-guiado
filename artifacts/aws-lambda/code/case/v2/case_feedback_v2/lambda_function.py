...
## Rol
Eres un evaluador académico experto en {nombre_curso}. Debes redactar una retroalimentación final, integral y personalizada para un estudiante que ha completado un reto de evaluación basado en un caso clínico.
...
IMPORTANTE:
- Cada intento incluye el fragmento del caso sobre el que se formuló la pregunta.
- Analiza si el estudiante aplicó correctamente los datos del caso en su respuesta.
- Detecta si mejoró, se estancó o repitió los mismos errores entre intentos.
- Identifica qué criterios se mantuvieron en nivel "bajo" a lo largo de los intentos.
- Usa los niveles cualitativos por criterio como evidencia objetiva, no los scores numéricos.
...
1. ANALIZA la progresión del estudiante considerando:
   - Si aplicó los datos del fragmento del caso para fundamentar sus respuestas.
   - Evolución (o ausencia de ella) por criterio entre intentos.
   - Si incorporó los feedbacks previos recibidos.
...
2. REDACTA un párrafo único con las siguientes características:
   ...
   - Menciona explícitamente los criterios o temas donde hubo debilidad persistente, referenciando el caso cuando sea relevante.
   - Si hubo mejora entre intentos, reconócela de forma natural sin mencionar números de intento.
   - Integra los errores como sugerencias de mejora constructivas ancladas en el caso.
...
- NO inventes datos del caso que no estén presentes en el fragmento.
...
- Se incluye una recomendación concreta anclada en el caso.
...
def _format_intentos(intentos: list, criterios: list, umbral: float) -> str:
    ...
    for intento in intentos:
        ...
        block = (
            ...
            f"  fragmento_caso: {intento.get('Contexto', '').strip()}\n"
...
def _validate_intentos(intentos: list) -> str | None:
    ...
    for intento in intentos:
        required = ("NumeroIntento", "Titulo", "Contexto", "Pregunta", "RespuestaModelo", "RespuestaUsuario", "Feedback", "Scores")
        if not all(k in intento for k in required):
            return f"Cada intento debe contener: {', '.join(required)}."

        if not isinstance(intento.get("Contexto", ""), str) or not intento["Contexto"].strip():
            return f"El campo 'Contexto' del intento {intento.get('NumeroIntento', '?')} debe ser un texto no vacío."

        scores = intento.get("Scores", [])
        if not isinstance(scores, list) or len(scores) == 0:
            return f"El campo 'Scores' del intento {intento.get('NumeroIntento', '?')} debe ser una lista no vacía."
...
def lambda_handler(event, context):
    try:
        logger.info(f"Event case general feedback: {event}")
        ...
        intentos_formateados  = _format_intentos(intentos, criterios, umbral)
        ...
        logger.error(f"Error in case general feedback: {str(e)}")
        return {
            "statusCode": 500,
