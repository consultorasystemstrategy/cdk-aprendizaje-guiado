...
# El score es una medición objetiva de 0 a 1, independiente del umbral. Los anclajes de la escala ahí son correctos y no deben depender del umbral configurado por el docente.
SCORE_PROMPT = """
## Rol
...
- Evalúa SOLO lo que el estudiante escribió (no infieras conocimiento implícito).
...
- No otorgues puntaje alto si solo hay definiciones generales sin explicación del proceso.
...
FEEDBACK_ALL_PROMPT = """
## Rol
Eres un evaluador académico experto en {nombre_curso}. Debes generar retroalimentación precisa y una nueva oportunidad de aprendizaje.
...
- Sé específico: menciona conceptos concretos (no generalidades).
...
- Debe ser una pregunta abierta que obligue a explicar procesos (no definiciones).

---

## Validación interna antes de responder
- El feedback menciona al menos un criterio débil.
- El reto aborda explícitamente esos vacíos.
...
- Si hay criterios con nivel "medio" (score entre {limite_medio} y {umbral}), menciona brevemente qué podría profundizar.
- Si hay evolución respecto a intentos anteriores, intégrala naturalmente.
- Incluye UNA sugerencia concreta para profundizar (no genérica).
...
- La sugerencia es específica (concepto o proceso).
...
- Evalúa la respuesta del estudiante considerando los siguientes criterios:
  ...
  5. Relevancia con respecto a los criterios de evaluación.
...
- Considera como válidas las respuestas que expresen correctamente los criterios aunque usen distinta redacción.
...
- Solo asigna un puntaje > 0.00 cuando exista contenido académico válido y relacionado con los criterios.
...
{intentos_anteriores_section}
## Instrucciones para el modelo:
1. REDACTA una retroalimentación breve en un máximo de 3 líneas, dirigida en segunda persona del singular (tú):
   - Señala con precisión qué concepto o criterio no fue comprendido o fue respondido de forma incompleta.
   - Si la respuesta fue parcialmente correcta, indica qué aspectos fueron acertados y qué faltó desarrollar.
   - Si no evidenció comprensión, explica en términos simples el concepto central que debía aplicarse.
   - No uses frases genéricas como "debes mejorar" o "intenta de nuevo". Sé específico con el contenido.
   - Si hay intentos anteriores, considera el patrón de errores acumulado para hacer la retroalimentación más precisa.

2. FORMULA un nuevo reto distinto al original y a cualquier intento anterior:
   - El nuevo reto debe abordar los mismos criterios desde un ángulo diferente al de todas las preguntas anteriores.
   - Si el error fue parcial, mantén la complejidad pero enfoca la pregunta en el aspecto omitido.
   - Si no demostró comprensión, reduce ligeramente la complejidad y formula una pregunta más directa y acotada.
   - La pregunta debe ser abierta e integradora, relacionando los criterios implicados.
   - La respuesta modelo debe responder directamente la nueva pregunta, con estructura clara y lenguaje técnico.
   - El título debe anticipar el enfoque del reto en máximo 6 palabras.
...
{{
  "feedback": "Retroalimentación breve en segunda persona, máximo 3 líneas",
  "reto": {{
    "titulo": "Título breve del nuevo reto",
    "pregunta": "Nueva pregunta abierta e integradora basada en los criterios",
...
- RECONOCE de manera directa qué criterio o proceso fue comprendido correctamente en esta respuesta.
- Si hubo intentos anteriores, reconoce la evolución del estudiante señalando en qué mejoró respecto a sus respuestas previas. No menciones los intentos de forma numérica ni técnica, intégralo de forma natural.
...
{{
  "feedback": "Retroalimentación en segunda persona, máximo 3 líneas"
}}
"""
'''
...
def _format_criterios(criterios: list) -> str:
    """
    Formatea criterios en estructura semi-JSON para mejorar parsing del LLM
    """
    blocks = []
...
def _validate_intentos_anteriores(intentos: list) -> str | None:
    """
    Valida la lista de intentos anteriores.
    Lista vacía es válida para el primer intento.
    """
...
def _validate_scores_criterios(scores_criterios: list, criterios: list) -> str | None:
    """
    Valida que el LLM haya retornado un score por cada CriterioId esperado.
    """
...
def lambda_handler(event, context):
    try:
        logger.info(f"Event path evaluate: {event}")
        ...
        reto = body["Reto"]
        required_reto_fields = ["Titulo", "Pregunta", "RespuestaModelo", "RespuestaUsuario", "Criterios"]
        ...
        logger.error(f"Error in path evaluate: {str(e)}")
        return {
            "statusCode": 500,
...
def lambda_handler(event, context):
    try:
        ...
        criterios_formateados = _format_criterios(criterios)
        intentos_anteriores_section = _build_intentos_anteriores_section(intentos_anteriores)
        ...
        primera_linea = score_raw.strip().splitlines()[0]
        match = re.match(r"^\s*([0-9]*\.?[0-9]+)\s*$", primera_linea)

        if not match:
            # No se pudo parsear el score: persistir y retornar error parcial
            _upload_evaluation(
                usuario_id=usuario_id,
                reto_iteracion_id=reto_iteracion_id,
                silabo_id=silabo_id,
            ...
            return {
                ...
                "body": json.dumps({
                    "success": False,
                    "message": "No se pudo determinar el puntaje de la respuesta."
                })
            }
        ...
        feedback_response = _invoke_prompt(prompt=feedback_prompt, max_tokens=LLM_MAX_TOKENS, temperature=0.7)
        feedback_raw = feedback_response["output"]["message"]["content"][0]["text"]
        input_tokens = feedback_response["usage"]["inputTokens"]
        output_tokens = feedback_response["usage"]["outputTokens"]

        # PASO 3: Persistir en DynamoDB (siempre, independiente del parseo)
        _upload_evaluation(
            usuario_id=usuario_id,
            reto_iteracion_id=reto_iteracion_id,
            silabo_id=silabo_id,
            unidad_id=unidad_id,
            sesion_id=sesion_id,
            score_promedio=str(score),
            prompt=feedback_prompt,
            ai_result=feedback_raw,
            input_tokens=input_tokens,
            output_tokens=output_tokens
        )
